...
class remedial_maintenance():
   ...
   # List View
   created_remedial_task_in_listView_xpath = '//div[@class="user-name"]//span[@class="tb-lead"]'
   ...
   def add_remedial_maintenance(self):
       element = self.driver.find_element(By.XPATH, self.add_remedial_maintenance_xpath)
       element.click()
       title = WebDriverWait(self.driver, 20).until(
           EC.presence_of_element_located((By.TAG_NAME, "title"))
       )
   ...
   def attachments(self):
       ...
       self.msg=WebDriverWait(self.driver, 20).until(EC.visibility_of_element_located((By.XPATH,'//div[@class="toastr-text"]//p[text()="File uploaded successfully"')))
   ...
   def remedial_mandatory_fields(self):
       ...
       validation_message = ["Title is required","Plant name is required","Field Engineer is required","Assigned To is required"]
   ...
   def remedial_task_in_listView(self,plant):
       element = WebDriverWait(self.driver, 10).until(EC.presence_of_all_elements_located((By.XPATH,self.created_remedial_task_in_listView_xpath)))
       for x in element:
           if x.text == plant:
               assert True
           break
   ...
   def view_remedial_task(self,plant):
       ...
       for x in element:
           if x.text in plant:
               x.click()
               time.sleep(3)
               title = WebDriverWait(self.driver, 50).until(EC.presence_of_element_located((By.TAG_NAME, "title")))
   ...
   def edit_mandatory_field(self):
        clear_mandatory_field = [self.task_name_xpath,self.assigned_to_xpath] #need to be add : self.field_engineer_xpath,
        for validation in clear_mandatory_field:
           element = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.XPATH,validation)))
           actions = ActionChains(self.driver)
        ...
        validation_message = ["Title is required","Assigned To is required"] #need to add :  "Field Engineer is required"
        for y in validation_message:
           element = WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located((By.XPATH,'//span[normalize-space()="'+y+'"]')))
           if element.is_displayed():
               assert True
           else:
   ...
   def edit_plant_dropdown(self,ticket):
        self.driver.find_element(By.XPATH,'(//div[normalize-space()="'+ticket+'"]/following::div[@class="dropdown"])[1]').click()
        element= WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.XPATH,'//div[@class="dropdown show"]//ul[@class="link-list-opt no-bdr"]//a[@id="edit-remedial"]')))
        element.click()
        title = WebDriverWait(self.driver, 20).until(
           EC.presence_of_element_located((By.TAG_NAME, "title"))
       )
   ...
   def archive_ticket(self,ticket):
        self.driver.find_element(By.XPATH,'(//div[normalize-space()="'+ticket+'"]/following::div[@class="dropdown"])[1]').click() 
        element= WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.XPATH,'//div[@class="dropdown show"]//ul[@class="link-list-opt no-bdr"]//a[@id="archive-remedial"]')))
        element.click()
        self.msg=WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located((By.XPATH,'//div[@class="toastr-text"]//p[@normalize-space()="Remedial Task Archived successfully"]')))
   ...
   def view_archive_list(self):
        ...
        element= WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.XPATH,'//label[@for="isArchived"]')))
        element.click()
   ...
   def unarchived_ticket(self,ticket):
        self.driver.find_element(By.XPATH,'(//div[normalize-space()="'+ticket+'"]/following::div[@class="dropdown"])[1]').click() 
        element= WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.XPATH,'//ul[@class="link-list-opt no-bdr"]//a[@id="unarchive-remedial"]')))
        element.click()
        self.msg=WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located((By.XPATH,'//div[@class="toastr-text"]//p[@normalize-space()="Remedial Task UnArchived successfully"]')))
   ...
   def unarchived_ticket_listView(self,ticket):
        self.driver.find_element(By.ID,"filter-remedial").click()
        reset_btn= WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.XPATH,"//button[normalize-space()='Reset Filter']")))
        reset_btn.click()
        ...
        for x in element:
            if x.text == ticket:
                ...
                assert False   
   def tickets_listview_count(self):
        count =0
